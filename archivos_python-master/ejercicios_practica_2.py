...
def desafio():
    ...
    addstock = str(input("que producto desea agregar al stock: ")).lower()
    stock = {'tornillos': 0, 'tuercas': 0, 'arandelas': 0}
    while True:
        if addstock == "fin":
            print("estos son los productos y su cantidad: ",stock)
            print("operacion terminada")
            break
        elif addstock in stock:
            nuevo_ingreso = int(input("ingrese cuanto stock desea agregar: "))
            stock[addstock] += nuevo_ingreso
            print(stock)
            return stock
            break
